...
package per.chaos.business.gui.index.tts;
...
import per.chaos.business.gui.common.dialogs.prompt.SecondaryConfirmDialog;
import per.chaos.business.gui.index.tts.table_action.TTSCardActionTableCellEditor;
import per.chaos.business.gui.index.tts.table_action.TTSCardActionTableCellRenderer;
...
import per.chaos.infra.runtime.models.tts.callback.TimbreSelectable;
...
public class TTSManagerDialog extends JDialog {
    ...
    public TTSManagerDialog(Window owner, FileCardCtx fileCardCtx) {
        ...
        ttsTable.setDefaultEditor(FileCard.class, new TTSCardActionTableCellEditor(new TTSCardButtonAction() {
            ...
            public void delete(FileCard fc, final AtomicReference<AudioPlayer> player) {
                final TTSManageService ttsManageService = BeanManager.inst().getReference(TTSManageService.class);
                Long fileReferDbId = fileCardCtx.getRawFileRefer().getFileRefer().getId();
                final File ttsAudioFile = ttsManageService.getTTSAudioFile(fileReferDbId, fc.getText());
    ...
    private void changeCurrentTimbreLabel() {
        String voiceId = fileCardCtx.getRawFileRefer().getFileRefer().getTimbre();
    ...
    private String getTTSFileStatistics(List<FileCard> fileCards) {
        final long hasDownloadAudioCount = fileCards.stream()
                ...
                .count();
        return "本地音频文件：" + hasDownloadAudioCount + "/" + fileCardCtx.getCardSize();
    }
    ...
    private List<FileCard> listFileCards() {
        List<FileCard> fileCards = fileCardCtx.getOriginalCards().stream().collect(Collectors.toList());
        final TTSManageService ttsManageService = BeanManager.inst().getReference(TTSManageService.class);
        Long fileReferDatabaseId = fileCardCtx.getRawFileRefer().getFileRefer().getId();
    ...
    private void downloadSingleTTSFiles(FileCard fileCard) {
        FileReferEntity fileRefer = fileCardCtx.getRawFileRefer().getFileRefer();
        ...
        ttsManageService.backgroundDownloadTTSFile(
                ...
                (downloadResult) -> {
                    ...
                    for (FileCard fc : fileCardCtx.getOriginalCards()) {
    ...
    private void downloadAllTTSFilesByVoiceId(final Long voiceId) {
        ...
        labelTTSStatistics.setText("正在下载中...");
        downloadProgressBar.setString("正在下载中...");
        downloadProgressBar.setToolTipText("正在下载中...");
        // 禁止界面操作按钮
        buttonDeleteAll.setEnabled(false);
        buttonDownload.setEnabled(false);
        ...
        ttsManageService.backgroundDownloadTTSFiles(
                ...
                (downloadResult) -> {
                    ...
                    // 界面展示文字刷新
                    downloadProgressBar.setValue(downloadProgress.get());
                    final String progressdisplayString = downloadProgress.get() + "/" + fileCardCtx.getCardSize();
                    downloadProgressBar.setString(progressdisplayString);
                    downloadProgressBar.setToolTipText("正在下载中..." + progressdisplayString);
                    ...
                    for (FileCard fileCard : fileCardCtx.getOriginalCards()) {
                ...
                (downloadAllComplete) -> {
                    ...
                    String promptContent = String.format(
                            ...
                            fileCardCtx.getFileName(), fileCardCtx.getCardSize(), downloadSuccessCnt, downloadFailCnt
                    );
    ...
    private void deleteAllTTSFile() {
        final TTSManageService ttsManageService = BeanManager.inst().getReference(TTSManageService.class);
        final String fileReferTTSFileRootFolder = ttsManageService.getFileReferTTSAbsolutePath(fileCardCtx.getRawFileRefer().getFileRefer().getId());
    ...
    private void timbreTest(ActionEvent e) {
        final Consumer<TimbreSelectable> timbreTestCallback = (timbreSelectable) -> {
            ...
            String currentVoiceId = fileCardCtx.getRawFileRefer().getFileRefer().getTimbre();
